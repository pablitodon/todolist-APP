...
const TodoList = ({ todos, setTodo, logAction }) => {
    ...
    const handleSave = (text, id) => {
        setEdit(null);
        logAction(`Задача изменена: ${text}`);
    ...
    const handleSuccess = (id, text, success) => {
        ...
        fetchSuccess();
        setTodo(todos.map(item => {
            return item.id === id ? { ...item, success: !item.success } : item;
        }))
        if (!success) {
                            ...
                            <div>
                                {
                                    isEdit === task.id ?
                                        <div>
                                            ...
                                            />
                                            <button onClick={() => handleSave(task.text, task.id)}>Save</button>
                                        </div>
                                        :
                                        <div>
                                            <div onClick={() => handleSuccess(task.id, task.text, task.success)}>
                                                {task.success ? <s className='text-gray-600/25'>{task.text}</s> : <p className='font-bold'>{task.text}</p>}
                                            </div>
                                            ...
                                            <button onClick={() => deleteTask(task.id, task.text)} >
